...
async def get_ges_district_extremes(ilce_adi: str, senaryo: str = Query(default="varsayilan")):
    ...
    max_scores = await run_in_threadpool(get_pixel_kriter_scores, float(r_max["lon"]), float(r_max["lat"]), "GES")
    min_scores = await run_in_threadpool(get_pixel_kriter_scores, float(r_min["lon"]), float(r_min["lat"]), "GES")
